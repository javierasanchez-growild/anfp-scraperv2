...
def _get_retry(path, intentos=5):
    """El plan gratis limita ~10 req/min. Si pega el limite, espera y reintenta."""
    d = {}
    for _ in range(intentos):
        d = _get(path)
        if _limite(d):
            print("  rate limit, espero 65s...")
            time.sleep(65)
            continue
        return d
    return d
...
def main():
    ...
    for slug, lid in LEAGUES.items():
        for season in SEASONS:
            try:
                d = _get_retry("/standings?" + urllib.parse.urlencode(
                ...
                d = _get_retry("/players/topscorers?" + urllib.parse.urlencode(
                    {"league": lid, "season": season}))
