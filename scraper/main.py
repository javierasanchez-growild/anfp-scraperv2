...
def capturar_normalizado(con, fecha_captura):
    """Descarga Wikipedia y llena las tablas normalizadas + CSV de la web."""
    total = 0
    for fuente in WIKI_SOURCES:
        torneo, url, slug = fuente["torneo"], fuente["url"], fuente["slug"]
        try:
            log.info("WIKI %s -> %s", torneo, url)
            html = sources.descargar_html(url)
            normal = sources.tablas_normalizadas(html)
            try:
                web_res = web_export.exportar(html, fecha_captura, torneo, slug)
