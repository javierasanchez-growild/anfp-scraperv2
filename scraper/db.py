...
def guardar_normalizada(con, nombre_tabla, df, fecha_captura, torneo):
    ...
    if _tabla_existe(con, nombre):
        ...
        con.commit()
        # Distintas ligas traen columnas distintas: agrega las que falten.
        existentes = {r[1] for r in con.execute(f"PRAGMA table_info({nombre})")}
        for col in df.columns:
            if col not in existentes:
                con.execute(f'ALTER TABLE {nombre} ADD COLUMN "{col}"')
        con.commit()
